...
import static Misc.CustomClasses.Chess.PieceType.*;
...
public class ChessGame {
    ...
    private static final int NOT_FOLLOWING_TURNS = 0;
     ...
     * @param pawnJumping whether pawns can move two tiles for their first move or not
     */
    ChessGame(int boardSizeX, int boardSizeY, boolean kingRestriction,
    ...
    public void move(int x1, int y1, int x2, int y2) {
        ...
        if (mC.contains(new Point2D(x2, y2))) {
            // special conditions
            if (iP.getPieceType() == WHITE_PAWN && y2 - y1 == 2) { // non-negative
                iP.setSpecialMoves(iP.getSpecialMoves() + 1);
            }
            if (iP.getPieceType() == BLACK_PAWN && y1 - y2 == 2) { // non_negative
                iP.setSpecialMoves(iP.getSpecialMoves() + 1);
            }
    ...
    private void setNaturalSetting() {
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                ...
                if (i == 1) {
                    board[i][j] = new Piece(BLACK_PAWN);
                }
                if (i == 6) {
                    board[i][j] = new Piece(WHITE_PAWN);
                }
                if (i == 7) {
    ...
    private void setProperPieces(ArrayList<Piece> arr, int row) {
        if (row == 0) {
            arr.add(new Piece(BLACK_ROOK));
            arr.add(new Piece(BLACK_KNIGHT));
            arr.add(new Piece(BLACK_BISHOP));
            arr.add(new Piece(BLACK_QUEEN));
            arr.add(new Piece(BLACK_KING));
            arr.add(new Piece(BLACK_BISHOP));
            arr.add(new Piece(BLACK_KNIGHT));
            arr.add(new Piece(BLACK_ROOK));
        } else if (row == 6) {
            arr.add(new Piece(WHITE_ROOK));
            arr.add(new Piece(WHITE_KNIGHT));
            arr.add(new Piece(WHITE_BISHOP));
            arr.add(new Piece(WHITE_QUEEN));
            arr.add(new Piece(WHITE_KING));
            arr.add(new Piece(WHITE_BISHOP));
            arr.add(new Piece(WHITE_KNIGHT));
            arr.add(new Piece(WHITE_ROOK));
        }
    ...
    public boolean whiteInCheck() {
        if (whitePieceTypes().contains(new Piece(WHITE_KING))) {
    ...
    public boolean blackInCheck() {
        if (blackPieceTypes().contains(new Piece(BLACK_KING))) {
        ...
        for (Point2D p : pieceLocations()) {
            if (pieceOf(p).getPieceType().color() != currentColor) {
                ...
                for (Point2D point : getMoveCoordinates(pX, pY)) {
                    ...
                    board[pointY][pointX] = new Piece(WHITE_PAWN); // anything but null
    ...
    private ArrayList<Point2D> whiteKingLocations() {
        ...
        for (Point2D p : whitePieceLocations()) {
            int y = (int) p.getY();
            int x = (int) p.getX();
            if (board[y][x].getPieceType() == WHITE_KING) {
