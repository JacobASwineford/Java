...
import static Misc.CustomClasses.Chess.ChessGame.*;
...
public class ChessGUI extends Application {
    ...
    private StackPane blackKing() {
        ...
        sP.setOnMouseClicked(event -> {
            if (c.getCurrentTurn() != TURN_BLACK) {
                return;
            }
            deselectAll();
    ...
    private StackPane whiteKing() {
        ...
        sP.setOnMouseClicked(event -> {
            if (c.getCurrentTurn() != TURN_WHITE) {
                return;
            }
            deselectAll();
            int xLocation = (int) (event.getSceneX() / SQUARE_SIZE);
    ...
    private void displayBoard() {
        for  (int i = 0; i < c.getBoardState().length; i++) {
            for (int j = 0; j < c.getBoardState()[i].length; j++) {
                ...
                if (c.pieceOf(j, i).getPieceType() == WHITE_PAWN) {
                    whitePawns[i][j].setVisible(true);
                }
                if (c.pieceOf(j, i).getPieceType() == BLACK_PAWN) {
                    blackPawns[i][j].setVisible(true);
                }
                if (c.pieceOf(j, i).getPieceType() == WHITE_ROOK) {
                    whiteRooks[i][j].setVisible(true);
                }
                if (c.pieceOf(j, i).getPieceType() == BLACK_ROOK) {
                    blackRooks[i][j].setVisible(true);
                }
                if (c.pieceOf(j, i).getPieceType() == WHITE_KNIGHT) {
                    whiteKnights[i][j].setVisible(true);
                }
                if (c.pieceOf(j, i).getPieceType() == BLACK_KNIGHT) {
                    blackKnights[i][j].setVisible(true);
                }
                if (c.pieceOf(j, i).getPieceType() == WHITE_BISHOP) {
                    whiteBishops[i][j].setVisible(true);
                }
                if (c.pieceOf(j, i).getPieceType() == BLACK_BISHOP) {
                    blackBishops[i][j].setVisible(true);
                }
                if (c.pieceOf(j, i).getPieceType() == WHITE_KING) {
                    whiteKings[i][j].setVisible(true);
                }
                if (c.pieceOf(j, i).getPieceType() == BLACK_KING) {
                    blackKings[i][j].setVisible(true);
                }
                if(c.pieceOf(j, i).getPieceType() == WHITE_QUEEN) {
                    whiteQueens[i][j].setVisible(true);
                }
                if(c.pieceOf(j, i).getPieceType() == BLACK_QUEEN) {
