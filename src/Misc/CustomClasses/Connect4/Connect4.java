...
class Connect4 {
    ...
    int drop(int column, int value) {
        ...
        history.add(new int[] {column, row});
        return row;
    }
    ...
    /**
     * Undoes the previous move. The board can be undone indefinitely as long as
     * the length of the history is more than 0.
     *
     * @return the coordinates of the chip that was removed.
     *         Null if the history is empty.
     */
    int[] undo() {
        if (history.size() == 0) return null;

        int[] coor = history.getLast();
